...
def calc_predicted_CMIP6_metrics(CMIP6_txt_path, var_name, model_in, CMIP6_model_list, outlier_method='percentile',dist_type=None):
    ...
    for i, CMIP6_model in enumerate(CMIP6_model_list):
        ...
        if day_time:
            if dist_type!=None:
                var_tmp = pd.read_csv(f'{CMIP6_txt_path}/predicted_CMIP6_DT_filtered_by_VPD_{var_name}_{scenario}_{CMIP6_model}_{model_in}_{region["name"]}_{dist_type}.csv', na_values=[''], usecols=[model_in])
            else:
                var_tmp = pd.read_csv(f'{CMIP6_txt_path}/predicted_CMIP6_DT_filtered_by_VPD_{var_name}_{scenario}_{CMIP6_model}_{model_in}_{region["name"]}.csv', na_values=[''], usecols=[model_in])
        else:
            if dist_type!=None:
                var_tmp = pd.read_csv(f'{CMIP6_txt_path}/predicted_CMIP6_{var_name}_{scenario}_{CMIP6_model}_{model_in}_{region["name"]}_{dist_type}.csv', na_values=[''], usecols=[model_in])
            else:
    ...
    if day_time:
        if dist_type!=None:
            metrics.to_csv(f'{CMIP6_txt_path}/metrics_CMIP6_DT_{var_name}_{scenario}_{model_in}_{region["name"]}_{dist_type}.csv')
        else:
            metrics.to_csv(f'{CMIP6_txt_path}/metrics_CMIP6_DT_{var_name}_{scenario}_{model_in}_{region["name"]}.csv')
    else:
        if dist_type!=None:
            metrics.to_csv(f'{CMIP6_txt_path}/metrics_CMIP6_{var_name}_{scenario}_{model_in}_{region["name"]}_{dist_type}.csv')
        else:
    ...
    if model_in == 'CABLE':
        ...
        for i, CMIP6_model in enumerate(CMIP6_model_list):
            ...
            if day_time:
                if dist_type !=None:
                    var_tmp = pd.read_csv(f'{CMIP6_txt_path}/predicted_CMIP6_DT_filtered_by_VPD_{var_name}_{scenario}_{CMIP6_model}_{model_in}_{region["name"]}_{dist_type}.csv', na_values=[''], usecols=['CMIP6'])
                else:
                    var_tmp = pd.read_csv(f'{CMIP6_txt_path}/predicted_CMIP6_DT_filtered_by_VPD_{var_name}_{scenario}_{CMIP6_model}_{model_in}_{region["name"]}.csv', na_values=[''], usecols=['CMIP6'])
            else:
                if dist_type !=None:
                    var_tmp = pd.read_csv(f'{CMIP6_txt_path}/predicted_CMIP6_{var_name}_{scenario}_{CMIP6_model}_{model_in}_{region["name"]}_{dist_type}.csv', na_values=[''], usecols=['CMIP6'])
                else:
                    var_tmp = pd.read_csv(f'{CMIP6_txt_path}/predicted_CMIP6_{var_name}_{scenario}_{CMIP6_model}_{model_in}_{region["name"]}.csv', na_values=[''], usecols=['CMIP6'])
        ...
        if day_time:
            if dist_type !=None:
                metrics.to_csv(f'{CMIP6_txt_path}/metrics_CMIP6_DT_{var_name}_{scenario}_CMIP6_{region["name"]}_{dist_type}.csv')
            else:
                metrics.to_csv(f'{CMIP6_txt_path}/metrics_CMIP6_DT_{var_name}_{scenario}_CMIP6_{region["name"]}.csv')

        else:
            if dist_type !=None:
                metrics.to_csv(f'{CMIP6_txt_path}/metrics_CMIP6_{var_name}_{scenario}_CMIP6_{region["name"]}_{dist_type}.csv')
            else:
                metrics.to_csv(f'{CMIP6_txt_path}/metrics_CMIP6_{var_name}_{scenario}_CMIP6_{region["name"]}.csv')
...
def calc_predicted_CMIP6_diff_metrics(CMIP6_txt_path, var_name, model_in, CMIP6_model_list, outlier_method='percentile',dist_type=None):
    ...
    for i, CMIP6_model in enumerate(CMIP6_model_list):
        ...
        if day_time:
            if dist_type!=None:
                var_tmp = pd.read_csv(f'{CMIP6_txt_path}/predicted_CMIP6_DT_filtered_by_VPD_{var_name}_{scenario}_{CMIP6_model}_{model_in}_{region["name"]}_{dist_type}.csv', na_values=[''], usecols=[model_in])
                obs_tmp = pd.read_csv(f'{CMIP6_txt_path}/predicted_CMIP6_DT_filtered_by_VPD_{var_name}_{scenario}_{CMIP6_model}_obs_{region["name"]}_{dist_type}.csv', na_values=[''], usecols=['obs'])
            else:
                var_tmp = pd.read_csv(f'{CMIP6_txt_path}/predicted_CMIP6_DT_filtered_by_VPD_{var_name}_{scenario}_{CMIP6_model}_{model_in}_{region["name"]}.csv', na_values=[''], usecols=[model_in])
                obs_tmp = pd.read_csv(f'{CMIP6_txt_path}/predicted_CMIP6_DT_filtered_by_VPD_{var_name}_{scenario}_{CMIP6_model}_obs_{region["name"]}.csv', na_values=[''], usecols=['obs'])
        else:
            if dist_type!=None:
                var_tmp = pd.read_csv(f'{CMIP6_txt_path}/predicted_CMIP6_{var_name}_{scenario}_{CMIP6_model}_{model_in}_{region["name"]}_{dist_type}.csv', na_values=[''], usecols=[model_in])
                obs_tmp = pd.read_csv(f'{CMIP6_txt_path}/predicted_CMIP6_{var_name}_{scenario}_{CMIP6_model}_obs_{region["name"]}_{dist_type}.csv', na_values=[''], usecols=['obs'])
            else:
                var_tmp = pd.read_csv(f'{CMIP6_txt_path}/predicted_CMIP6_{var_name}_{scenario}_{CMIP6_model}_{model_in}_{region["name"]}.csv', na_values=[''], usecols=[model_in])
                obs_tmp = pd.read_csv(f'{CMIP6_txt_path}/predicted_CMIP6_{var_name}_{scenario}_{CMIP6_model}_obs_{region["name"]}.csv', na_values=[''], usecols=['obs'])
    ...
    if day_time:
        if dist_type!=None:
            metrics.to_csv(f'{CMIP6_txt_path}/diff_metrics_CMIP6_DT_{var_name}_{scenario}_{model_in}_{region["name"]}_{dist_type}.csv')
        else:
            metrics.to_csv(f'{CMIP6_txt_path}/diff_metrics_CMIP6_DT_{var_name}_{scenario}_{model_in}_{region["name"]}.csv')
    else:
        if dist_type!=None:
            metrics.to_csv(f'{CMIP6_txt_path}/diff_metrics_CMIP6_{var_name}_{scenario}_{model_in}_{region["name"]}_{dist_type}.csv')
        else:
            metrics.to_csv(f'{CMIP6_txt_path}/diff_metrics_CMIP6_{var_name}_{scenario}_{model_in}_{region["name"]}.csv')
