...
for var_name2 in "${var_name2s[@]}"; do
  ...
  for IGBP_type in ${IGBP_types[@]}; do
    ...
    site_scripts+=("process_step4_fit_curve_${case_name}.py")
...
#
# for site_script in "${site_scripts[@]}"; do
#   echo "$site_script"
# done

    cd /g/data/w97/mm3972/scripts/PLUMBER2/LSM_VPD_PLUMBER2/submit

    cat > "process_step4_fit_curve_${case_name_no_site}.sh" << EOF_submit
...
printf "%s\n" "${site_scripts[@]}" | xargs -n 1 -P 104 python

EOF_submit

    qsub "process_step4_fit_curve_${case_name_no_site}.sh"
