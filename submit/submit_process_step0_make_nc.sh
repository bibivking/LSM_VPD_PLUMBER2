...
def add_Rnet_obs_to_nc_file(PLUMBER2_flux_path, site_name, output_file):
    ...
    f_in               = nc.Dataset(file_path[0])
    Rnet               = f_in.variables['Rnet'][:]

    Rnet               = np.where(Rnet>2000., np.nan, Rnet)
    Rnet               = np.where(Rnet<-1000., np.nan, Rnet)

    f_out              = nc.Dataset(output_file,'r+', format='NETCDF4')

    if 'obs_Rnet' not in f_out.variables:
        obs                = f_out.createVariable('obs_Rnet', 'f4', ('CABLE_time'))
        obs.standard_name  = "obs_net_radiation"
        obs.long_name      = "Net radiation"
        obs.units          = "W/m2"
        obs[:]             = Rnet
    else:
        f_out.variables['obs_Rnet'][:] = Rnet
...
def add_Rnet_to_nc_file(PLUMBER2_path, model_names, site_name, output_file):

    '''
    Add modelled Rnet to the netcdf file
    '''

    varname        = "Rnet"
    var_name_dict  = check_variable_exists(PLUMBER2_path, varname, site_name, model_names)
    ...
    for j, model_name in enumerate(model_names):
        ...
        if len(var_name_tmp) == 2 or len(var_name_tmp) == 3:
            var_name_first = var_name_tmp[0]
        else:
            var_name_first = var_name_tmp

        # if the variable exists
        if not ('None' in var_name_tmp):
            print(model_name,var_name_tmp)
            ...
            # Read variable attributions info from input
            # if not var_long_name:
            if isinstance(var_name_dict[model_name], list):
                var_long_name = 'Net radiation (calculated from ' + ', '.join(var_name_dict[model_name]) + ')'
            else:
                var_long_name = 'Net radiation (calculated from ' + var_name_dict[model_name] + ')'

            print('var_long_name',var_long_name)

            # Read Rnet from input
            if len(var_name_tmp) == 2:
                # Rnet = SWnet + LWnet
                Var_tmp = f.variables[var_name_tmp[0]][:]+f.variables[var_name_tmp[1]]
            elif len(var_name_tmp) == 3:
                # Rnet = Qle + Qh + Qg
                Var_tmp = f.variables[var_name_tmp[0]][:]+f.variables[var_name_tmp[1]]+f.variables[var_name_tmp[2]]
            else:
                # Rnet = Rnet
                Var_tmp = f.variables[var_name_tmp][:]
            ...
            # Reset missing value
            for attr in ['_FillValue', '_fillValue', 'missing_value']:
                if hasattr(f.variables[var_name_first], attr):
                    var_FillValue = getattr(f.variables[var_name_first], attr)
                    break
            else:
                var_FillValue = None
            ...
            if hasattr(f.variables[var_name_first], 'dimensions'):
                if 'patch' in f.variables[var_name_first].dimensions:
                    # print('model_name', model_name, 'site_name', site_name,'has patch demension' )
                    patch = f.dimensions['patch'].size
                    # print('patch',patch)

            if hasattr(f.variables[var_name_first], 'dimensions'):
                if 'veget' in f.variables[var_name_first].dimensions:
                    # print('model_name', model_name, 'site_name', site_name,'has veget demension' )
                    veget = f.dimensions['veget'].size
                    # print('veget',veget)
            ...
            # Read variable units
            model_var_units[model_name] = f.variables[var_name_first].units

            # ===== Quality Control =====
            Var_tmp_tmp = np.where(Var_tmp_tmp>2000., np.nan, Var_tmp_tmp)
            Var_tmp_tmp = np.where(Var_tmp_tmp<-1000., np.nan, Var_tmp_tmp)
...
def add_SM_topXm_to_nc_file(PLUMBER2_path, output_file, site_name, SM_names, soil_thicknesses, top_thickness=1.):

    '''
    Give the soil moisture in the top X m (0.3, 0.5, or 1.0 m) to every model. For the models have simulated soil
    ...
    for n, model_name in enumerate(model_names):
        ...
        if not file_path:
            ...
            # Calculate SM in the top top_thickness meter
            i      = 0
            SM_tmp = 0

            # replace 0 values with 0.5 (kg/m2) in STEMMUS-SCOPE
            # otherwises, these 0 values will lead to inf when standardize Qle by SMtop1m
            if model_name == 'STEMMUS-SCOPE':
                Var_tmp_tmp = np.where(Var_tmp_tmp == 0, 0.5, Var_tmp_tmp)

            # total soil layers thickness >= top_thickness
            if np.sum(soil_thickness) >= top_thickness:
                # For soil layers shallower than top_thickness
                while soil_thickness_acl[i] <= top_thickness:
                ...
                if soil_thickness_acl[i-1] < top_thickness and soil_thickness_acl[i] > top_thickness:
                    if model_name == 'MuSICA' or model_name == 'STEMMUS-SCOPE':
                        if units_kg_m_2:
                            try:
                                print('if Var_tmp_tmp[:,0,0,i], site_name=',site_name)
                                SM_tmp = SM_tmp + Var_tmp_tmp[:,0,0,i]*(top_thickness-soil_thickness_acl[i-1])/soil_thickness[i]
                            except:
                                # CA-NS1 and AR-SLu in STEMMUS-SCOPE has different dimension setting than other sites
                                print('if Var_tmp_tmp[:,i,0,0], site_name=',site_name)
                                SM_tmp = SM_tmp + Var_tmp_tmp[:,i]*(top_thickness-soil_thickness_acl[i-1])/soil_thickness[i]
                        else:
                            SM_tmp = SM_tmp + Var_tmp_tmp[:,0,0,i]*(top_thickness-soil_thickness_acl[i-1])
                    else:
                        if units_kg_m_2:
                            SM_tmp = SM_tmp + Var_tmp_tmp[:,i]*(top_thickness-soil_thickness_acl[i-1])/soil_thickness[i]
                        else:
                            SM_tmp = SM_tmp + Var_tmp_tmp[:,i]*(top_thickness-soil_thickness_acl[i-1])

                # Change from mm*m2/m3 to m3/m3
                if units_kg_m_2:
                    SM_tmp = SM_tmp/1000./top_thickness
                else:
                    SM_tmp = SM_tmp/top_thickness
            ...
            SM               = f_out.createVariable(model_name+'_SMtop'+str(top_thickness)+'m', 'f4', (model_name+'_time'))
            SM.standard_name = f"Soil moisture in the top {top_thickness} m (root zone moisture)"
            SM.long_name     = f"Soil moisture in the top {top_thickness} m (root zone moisture)"
            SM.units         = "m3/m3"
            SM[:]            = SM_tmp_1D
    ...
    # Add the soil moisture values to the output nc file
    SM               = f_out.createVariable('model_mean_SMtop'+str(top_thickness)+'m', 'f4', ('CABLE_time'))
    SM.standard_name = f"Mean soil moisture in the top {top_thickness} m (root zone moisture)"
    SM.long_name     = f"Mean soil moisture in the top {top_thickness} m (root zone moisture)"
    SM.units         = "m3/m3"
    SM[:]            = SM_mean
    f_out.close()
...
if __name__ == "__main__":
    ...
    for site_name in site_names:
        ...
        add_Rnet_to_nc_file(PLUMBER2_path, model_names, site_name, output_file)
        gc.collect()

        add_Rnet_obs_to_nc_file(PLUMBER2_flux_path, site_name, output_file)
        gc.collect()

        # Function add_SM_top1m_to_nc_file content some models are not used in the paper
        # it will lead to crash, so comment it out
        top_thickness = 0.3
        add_SM_topXm_to_nc_file(PLUMBER2_path,output_file,site_name,SM_names,soil_thicknesses,top_thickness)
        gc.collect()
