...
cat > save_predicted_CMIP6_3hourly_${scenario}_${CMIP6_model}_north_Am.py << EOF_predicted_CMIP6
#!/usr/bin/env python
...
def calc_predicted_CMIP6_each_model(CMIP6_txt_path, scenario, CMIP6_model, model_in, var_name='Qle', day_time=False,
    ...
    # Read data
    if day_time:
        var_output = pd.read_csv(f'{CMIP6_txt_path}/save_csv/CMIP6_DT_filtered_by_VPD_{var_name}_{scenario}_{CMIP6_model}_{region["name"]}.csv', na_values=[''])
    else:
        var_output = pd.read_csv(f'{CMIP6_txt_path}/save_csv/CMIP6_{var_name}_{scenario}_{CMIP6_model}_{region["name"]}.csv', na_values=[''])
    ...
    if day_time:
        if dist_type == None:
            Qle_pred.to_csv(f'{CMIP6_txt_path}/predict/predicted_CMIP6_DT_filtered_by_VPD_{var_name}_{scenario}_{CMIP6_model}_{model_in}_{region["name"]}.csv')
        else:
            Qle_pred.to_csv(f'{CMIP6_txt_path}/predict/predicted_CMIP6_DT_filtered_by_VPD_{var_name}_{scenario}_{CMIP6_model}_{model_in}_{region["name"]}_{dist_type}.csv')
    else:
        if dist_type == None:
            Qle_pred.to_csv(f'{CMIP6_txt_path}/predict/predicted_CMIP6_{var_name}_{scenario}_{CMIP6_model}_{model_in}_{region["name"]}.csv')
        else:
            Qle_pred.to_csv(f'{CMIP6_txt_path}/predict/predicted_CMIP6_{var_name}_{scenario}_{CMIP6_model}_{model_in}_{region["name"]}_{dist_type}.csv')

    gc.collect()

    return

def save_predicted_CMIP6_3hourly_parallel(CMIP6_txt_path, model_list, scenario, CMIP6_model, var_name='Qle',
                                          day_time=False, region={'name':'global','lat':None, 'lon':None}, dist_type=None):
...
if __name__ == "__main__":
    ...
    dist_type   = 'Poisson'
    save_predicted_CMIP6_3hourly_parallel(CMIP6_txt_path, model_list, scenario, CMIP6_model,
                                          var_name=var_name, day_time=day_time, region=region,
                                          dist_type=dist_type)
EOF_predicted_CMIP6

cd /g/data/w97/mm3972/scripts/PLUMBER2/LSM_VPD_PLUMBER2/submit

cat > submit_predicted_CMIP6_${scenario}_${CMIP6_model}_north_Am.sh << EOF_submit
#!/bin/bash
...
cd /g/data/w97/mm3972/scripts/PLUMBER2/LSM_VPD_PLUMBER2
python save_predicted_CMIP6_3hourly_${scenario}_${CMIP6_model}_north_Am.py

EOF_submit

qsub submit_predicted_CMIP6_${scenario}_${CMIP6_model}_north_Am.sh
