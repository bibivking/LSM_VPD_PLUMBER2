...
def write_raw_data_var_VPD(var_name, site_names, PLUMBER2_path, bin_by=None, low_bound=30,
    ...
    if country_code!=None:
        if standardize == 'by_daily_obs_mean':
            var_output = pd.read_csv(f'./txt/all_sites_daily/{var_name}_all_sites_'+country_code+'_daily.csv',na_values=[''])
        else:
            var_output = pd.read_csv(f'./txt/all_sites/{var_name}_all_sites_'+country_code+'.csv',na_values=[''])
    else:
        if standardize == 'by_daily_obs_mean':
            var_output = pd.read_csv(f'./txt/all_sites_daily/{var_name}_all_sites_daily.csv',na_values=[''])
        else:
