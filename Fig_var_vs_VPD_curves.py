...
def plot_var_VPD_uncertainty(var_name=None, day_time=False, energy_cor=False, time_scale=None, country_code=None,
                 ...
                 VPD_num_threshold=200, middle_day=False, LAI_range=None, smooth_type='no_soomth'):
    ...
    if calc_correl:
        ...
        #     else:
        #         file_names = [  f'./txt/process4_output/{folder_name}/{var_name}{file_message_wet}_obs{site_info}.csv',
        #                         f'./txt/process4_output/{folder_name}/{var_name}{file_message_dry}_obs{site_info}.csv',]
    ...
    # # set uncertainty files
    # file_names_uncertain = \
    #     [f'./txt/process4_output/{folder_name}/{var_name}_hourly_RM16_DT_EF_model_0-0.2_CRV_bins_UCRTN_percentile.csv',
    #      f'./txt/process4_output/{folder_name}/{var_name}_hourly_RM16_DT_EF_model_0.8-1.0_CRV_bins_UCRTN_percentile.csv',]
    ...
    for j, model_out_name in enumerate(model_order):
        ...
        #     else:
        #         file_names = [  f'./txt/process4_output/{folder_name}/{var_name}{file_message_wet}_{model_out_name}{site_info}.csv',
        #                         f'./txt/process4_output/{folder_name}/{var_name}{file_message_dry}_{model_out_name}{site_info}.csv',]
        if select_site != None:
            if dist_type!=None:
                file_names = [  f'./txt/process4_output/{folder_name}/{var_name}{file_message_wet}_{dist_type}{site_info}.csv',
                                f'./txt/process4_output/{folder_name}/{var_name}{file_message_dry}_{dist_type}{site_info}.csv',]
            ...
            if dist_type!=None:
                ...
                file_names = [  f'./txt/process4_output/{folder_name}/{var_name}{file_message_wet}{site_info}{message_midday}.csv',
                                f'./txt/process4_output/{folder_name}/{var_name}{file_message_dry}{site_info}{message_midday}.csv',]

        # print(file_names)

        # # Read data uncertainties
        # uncertain_dry_tmp     = pd.read_csv(file_names_uncertain[0],usecols=['vpd_series',model_out_name+'_vpd_num',model_out_name+'_vals',model_out_name+'_top',model_out_name+'_bot'])
        # uncertain_wet_tmp     = pd.read_csv(file_names_uncertain[1],usecols=['vpd_series',model_out_name+'_vpd_num',model_out_name+'_vals',model_out_name+'_top',model_out_name+'_bot'])
        #
        # uncertain_dry_vpd_num = uncertain_dry_tmp[model_out_name+'_vpd_num'].values
        # uncertain_wet_vpd_num = uncertain_wet_tmp[model_out_name+'_vpd_num'].values
        #
        # dry_tmp         = np.where(uncertain_dry_vpd_num>=VPD_num_threshold, 1, 0)
        # dry_index       = np.argwhere(dry_tmp==1)[-1]
        # uncertain_dry   = uncertain_dry_tmp.loc[:dry_index[0]]
        #
        # wet_tmp         = np.where(uncertain_wet_vpd_num>=VPD_num_threshold, 1, 0)
        # wet_index       = np.argwhere(wet_tmp==1)[-1]
        # uncertain_wet   = uncertain_wet_tmp.loc[:wet_index[0]]
        #
        # uncertain_dry_vpd     = uncertain_dry['vpd_series']
        # uncertain_wet_vpd     = uncertain_wet['vpd_series']
        # # uncertain_dry_vals    = uncertain_dry[model_out_name+'_vals']
        # # uncertain_wet_vals    = uncertain_wet[model_out_name+'_vals']
        # uncertain_dry_top     = uncertain_dry[model_out_name+'_top']
        # uncertain_wet_top     = uncertain_wet[model_out_name+'_top']
        # uncertain_dry_bot     = uncertain_dry[model_out_name+'_bot']
        # uncertain_wet_bot     = uncertain_wet[model_out_name+'_bot']
        ...
        for i, file_name in enumerate(file_names):
            ...
            if os.path.exists(file_name):
                ...
                # # Plotting GAM models' uncertainty
                # # add error range of obs (I think top and bot boundary should be set to 1 sigema)
                # if col == 0:
                #     fill = ax[row,col].fill_between(uncertain_wet_vpd,uncertain_wet_bot,uncertain_wet_top,
                #             color=line_color, edgecolor="none", alpha=0.1) #  .rolling(window=10).mean()
                # elif col == 1:
                #     fill = ax[row,col].fill_between(uncertain_dry_vpd,uncertain_dry_bot,uncertain_dry_top,
                #             color=line_color, edgecolor="none", alpha=0.1) #  .rolling(window=10).mean()
                if col == 0:
                    # aridity_index = check_site_info(site_name,'aridity_index')
                    ax[row,col].text(0.05, 0.92, f'{orders[i]}', va='bottom', ha='center', rotation_mode='anchor',transform=ax[row,col].transAxes, fontsize=14)
                    #,{site_name},{aridity_index}
                else:
                    # IGBP_type = check_site_info(site_name,'IGBP_type')
                    # clim_type = check_site_info(site_name,'climate_type')
                    ax[row,col].text(0.05, 0.92, f'{orders[i]}', va='bottom', ha='center', rotation_mode='anchor',transform=ax[row,col].transAxes, fontsize=14)
                    # ,{clim_type},{IGBP_type}
    ...
    # # dry condition
    # fill = ax[0,0].fill_between(obs_wet_vpd,obs_wet_bot,obs_wet_top,
    #                 color=model_colors['obs'], edgecolor="none", alpha=0.3)
    # fill = ax[0,1].fill_between(obs_dry_vpd,obs_dry_bot,obs_dry_top,
    #                 color=model_colors['obs'], edgecolor="none", alpha=0.3)
    #
    # ax[0,0].set_title("Wet (EF>0.8)", fontsize=16)
    # ax[0,1].set_title("Dry (EF<0.2)", fontsize=16)

    ax[0,0].set_title("Wet (SM>85th)", fontsize=16)
    ax[0,1].set_title("Dry (SM<15th)", fontsize=16)
    ...
    if 'TVeg' in var_name or 'Qle' in var_name:
        if time_scale == 'daily':
            ...
            if middle_day:
                ...
                ax[0,0].set_ylim(0,250)
            elif 'SMtopXm' in standardize and 'Qle' in var_name:
                ax[0,0].set_ylim(0,600)
            elif standardize == 'STD_SMtop1m':
    ...
    if select_site!=None:
        site_plot = 'site_plots/'
    else:
        site_plot = ''

    if class_by !=None:
        fig.savefig(f"./plots/{site_plot}Fig_{var_name}_VPD{file_message_dry}_color_{class_by}{message}_gs_eq{site_info}{message_midday}.png",bbox_inches='tight',dpi=300) # '_30percent'
    else:
        fig.savefig(f"./plots/{site_plot}Fig_{var_name}_VPD{file_message_dry}{message}{site_info}{message_midday}.png",bbox_inches='tight',dpi=300) # '_30percent'
...
def plot_var_VPD_uncertainty_three_cols(var_name=None, day_time=False, energy_cor=False, time_scale=None, country_code=None,
    ...
    # # Read data uncertainties
    # file_names_uncertain = \
    #     [f'./txt/process4_output/{folder_name}/{var_name}_hourly_RM16_DT_EF_model_0.2-0.4_CRV_bins_UCRTN_percentile.csv',
    #      f'./txt/process4_output/{folder_name}/{var_name}_hourly_RM16_DT_EF_model_0.4-0.6_CRV_bins_UCRTN_percentile.csv',
    #      f'./txt/process4_output/{folder_name}/{var_name}_hourly_RM16_DT_EF_model_0.6-0.8_CRV_bins_UCRTN_percentile.csv',]
    #
    ...
    for j, model_out_name in enumerate(model_order):
        ...
        for i, file_name in enumerate(file_names):
            ...
            if os.path.exists(file_name):
                ...
                if j == 0 :
                    ax[row,col].text(0.05, 0.92, orders[i], va='bottom', ha='center', rotation_mode='anchor',transform=ax[row,col].transAxes, fontsize=14)
    ...
    if var_name == 'Qle':
        ...
        ax[0,0].set_ylabel("Transpriation (W m$\mathregular{^{-2}}$)", fontsize=12)
    ...
    if 'Qle' in var_name or 'TVeg' in var_name:
        if time_scale == 'daily':
    ...
    for j, model_out_name in enumerate(model_order):
        ...
        for i, file_name in enumerate(file_names):
            ...
            if os.path.exists(file_name):
                ...
                # if turning_point['calc']:
                #     ax[row,col].scatter(turning_points[model_out_name][0], turning_points[model_out_name][1], marker='o', color=line_color, s=20)
                ax[row,col].text(0.05, 0.90, orders[i], va='bottom', ha='center', rotation_mode='anchor',transform=ax[row,col].transAxes, fontsize=14)
...
if __name__ == "__main__":
    ...
    window_size    = 5
    order          = 3
    smooth_type    = 'S-G_filter' #'S-G_filter' #'no_soomth' #
    ...
    if 0:
        ...
        for site_name in site_names:
            if not site_name in clarify_site['remove_site']:
    ...
    if 1:
        selected_by    = 'SM_per_all_models'
        middle_day     = False
        standardize    = None #'STD_annual_model' # None #
        var_name       = 'Qle'
        LAI_range      = None
        add_Xday_mean_EF= None#'1'
        IGBP_type      = None
        for clim_type in clim_types:
            ...
            plot_var_VPD_uncertainty(var_name=var_name, day_time=day_time, energy_cor=energy_cor, time_scale=time_scale, country_code=country_code,
                            selected_by=selected_by, veg_fraction=veg_fraction, standardize=standardize, uncertain_type=uncertain_type, method=method,
                            IGBP_type=IGBP_type, clim_type=clim_type, clarify_site=clarify_site,num_threshold=num_threshold, class_by=class_by,
                            LAI_range = LAI_range, add_Xday_mean_EF=add_Xday_mean_EF, smooth_type=smooth_type,
                            dist_type=dist_type, calc_correl=calc_correl, select_site=select_site, middle_day=middle_day)
